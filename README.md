...
- **Maestr@**: campo opcional de la ficha de clase, **varios separados por coma**. Se muestra al lado del título, más chico y en naranja, como *“con Ana y Luis”* (`joinNames()` los enumera en castellano: coma entre los del medio y “y” antes del último). Vacío no muestra nada. Va dentro del `<h2>` para que fluya después del título —que puede tener varios renglones— con un espacio duro, así “con” no queda solo al final de un renglón y quien copie el texto o use un lector de pantalla no lee las dos cosas pegadas.
  En el formulario, "Maestr@" (`tags`) y "Lugar" (`suggest`) se completan eligiendo de [Datos recurrentes](#qué-se-puede-administrar) —chips para el maestro, `<datalist>` para el lugar— o escribiendo a mano. Lo que se guarda sigue siendo el mismo string, así que el registro y el render público no cambian; `SectionController::edit` sólo agrega el prop `pools` cuando el tipo tiene un campo con `pool`.
...
| **Datos recurrentes** | Dos listas —**maestr@s** (tabla `teachers`) y **lugares** (tabla `venues`)— que el dueño carga una vez para después elegirlas en los campos "Maestr@" y "Lugar" de las fichas de clase, sin reescribirlas. Sólo agregar y borrar; orden alfabético; nombre único. Las fichas siguen guardando el texto: la lista es sólo el menú de sugerencias, así que borrar un nombre no cambia las fichas que ya lo usan. `DatosRecurrentesSeeder` la llena con lo que ya está escrito en las fichas del sitio. |
| **Ayuda → Tutoriales** | Grupo aparte en el menú del panel. Videos de YouTube que explican cómo usar el panel, con nombre y enlace, ordenables como las FAQ (tabla `tutorials`). Se muestran embebidos (`youtube-nocookie`). Son **solo para quien administra**: no tocan el sitio público. `App\Support\YouTube` saca el id del video de cualquier forma de enlace (barra del navegador, "Compartir", `youtu.be`, Short, embed, id pelado) y arma la URL del iframe y la miniatura. |
...
- `ContentSeeder` — siembra el sitio completo (todas las páginas, FAQs, eventos, ajustes) desde `database/seeders/data/*.php`. Lo llama `DatabaseSeeder` junto con el usuario admin y `DatosRecurrentesSeeder`.
- **Seeders puntuales** (tocan **una sola página** sin pisar el resto del contenido ya editado — pensados para producción):
  ...
  php artisan db:seed --class=ClasesSemanalesFichaSeeder --force # la ficha plantilla de Clases semanales
  php artisan db:seed --class=DatosRecurrentesSeeder --force    # llena maestr@s y lugares desde las fichas
  ```
...
Incluye `tests/Feature/SiteContentTest.php`: render público, ocultar/eliminar secciones y páginas, la ficha plantilla no eliminable, permisos del admin, edición de secciones con reemplazo de imagen, validaciones, eventos en la home, FAQs compartidas y el calendario (clases semanales en todos sus días, eventos de varios días, `?mes=`, el día argentino y no UTC, el tilde masivo del panel). `tests/Feature/Auth/GoogleLoginTest.php` cubre la lista blanca del login con Google, `tests/Feature/TutorialsTest.php` el CRUD de tutoriales y `tests/Feature/RecurringDataTest.php` las listas de maestr@s y lugares (alta, baja, únicas, el prop `pools` del formulario y el seeder que las llena). En `tests/Unit/` están la expansión de las reglas de fecha (`OccurrencesTest`) y el parseo de enlaces de YouTube (`YouTubeTest`), con sus casos borde.
