...
- **Ancla para enlazar**: campo opcional de la ficha de clase, para apuntar un botón directo a una ficha (`/cursos-y-retiros#retiro-de-agosto`, o `#4` si preferís un número). Acepta letras, números, `-` y `_` empezando por letra o número —nada de espacios, acentos ni `#`, porque va dentro de una URL— y **tiene que ser única en la página**: si se repite, el navegador salta a la primera y la otra queda inalcanzable sin avisar, así que el guardado lo rechaza. El formulario muestra la URL ya armada debajo del campo. Las reglas propias del campo salen de una clave `rules` en el registro, que pisa las del tipo.
  Dos cosas que el ancla necesita y no son obvias: `scroll-mt-24` en la sección, porque el header es sticky (80 px) y sin eso el navegador deja la ficha justo debajo y el encabezado la tapa; y el composable `useHashScroll()` en `Public/Page.vue`, porque **al ser una SPA el navegador no puede hacerlo solo** — cuando busca el ancla, Vue todavía no dibujó la sección, y al llegar por un enlace de Inertia no hay navegación de documento que lo dispare. Medido: sin el composable sólo funcionaba cambiando el hash estando ya en la página. Vuelve a apuntar cuando terminan de cargar las imágenes, que al no declarar alto empujan el contenido.
